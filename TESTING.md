...
**Legenda piattaforme:**
- **T** = Tauri (app desktop, modalità locale)
- **W** = Web (browser, modalità locale)
- **S** = Server mode (richiede connessione al server)
...
## 2. AUTH - Autenticazione (Server Mode)
...
| AUTH-009  | Single-login: nuovo login invalida sessione       | - | x | x |
...
| PROJ-015  | Click su logo torna a splash/lista      | x | x | - |
...
| EDIT-003  | Redo locale (Cmd+Shift+Z) nel chunk     | x | x | - |
...
| EDIT-007  | Nuova modifica cancella redo stack      | - | x | x |
...
| STRUCT-007 | Elimina chunk con figli bloccato        | x | x | - |
...
| ASPECT-002 | Aggiungi nuovo aspetto (pers/luogo/etc) | x | x | - |
...
| ASPECT-009 | Elimina aspetto (rimuove da tutti)      | x | x | - |
...
| ID        | Test                                    | T | W | S |
|-----------|-----------------------------------------|---|---|---|
| ANNOT-001 | Aggiungi commento inline (Cmd+M)        | x | x | - |
...
| ANNOT-007 | Click annotazione naviga al testo       | x | x | - |
| ANNOT-008 | Risolvi/chiudi annotazione              | x | x | - |
| ANNOT-009 | Display name su commenti server         | - | x | x |
...
| PROP-002  | Proposta visibile come highlight        | - | x | x |
| PROP-003  | Click su proposta mostra popover + diff | - | x | x |
| PROP-004  | Owner accetta proposta (testo sostituito)| - | x | x |
| PROP-005  | Owner rifiuta proposta                  | - | x | x |
| PROP-006  | Proposta con caratteri speciali         | - | x | x |
| PROP-007  | Proposta su whitespace funziona         | - | x | x |
| PROP-008  | Pannello proposte mostra lista          | - | x | x |
| PROP-009  | Collaborator non può modificare diretto | - | x | x |
...
| DISCUSS-008 | Indicatore discussioni nell'outline   | - | x | x |
...
| CHAT-003  | Messaggi mostrano display name          | - | x | x |
...
| COLLAB-003  | Collaboratore vede progetto in lista  | - | x | x |
| COLLAB-004  | Collaboratore non può eliminare prog  | - | x | x |
| COLLAB-005  | Owner assegna chunk a collaboratore   | - | x | x |
| COLLAB-006  | Owner rilascia chunk da collaboratore | - | x | x |
| COLLAB-007  | Collaboratore modifica solo suoi chunk| - | x | x |
...
| EXP-006  | Esporta selezione (solo chunk selez.)   | x | x | - |
...
## Test Esclusivamente Manuali

I seguenti test **non possono essere automatizzati** e richiedono verifica manuale:

### Multi-utente Real-time
| ID | Test | Motivo |
|----|------|--------|
| COLLAB-008 | Sync real-time modifiche tra utenti | Richiede due browser/sessioni simultanee |
| COLLAB-009 | Conflict resolution (last-write-wins) | Timing critico multi-utente |
| COLLAB-010 | Presence indicator (chi sta editando) | Verifica visiva real-time |
| CHAT-004 | Polling nuovi messaggi funziona | Timing polling multi-utente |

### AI (richiedono API key)
| ID | Test | Motivo |
|----|------|--------|
| AI-001 | Settings: configura API key | Richiede credenziali reali |
| AI-002 | Settings: seleziona modello | Dipende da AI-001 |
| AI-003 | Genera testo con AI (Cmd+G) | Richiede API funzionante |
| AI-004 | AI rispetta contesto chunk corrente | Richiede API funzionante |
| AI-005 | Annulla generazione in corso | Richiede API funzionante |
| AI-006 | Errore API key invalida gestito | Richiede testing manuale errori |
| AI-007 | Streaming risposta visibile | Verifica visiva streaming |
| AI-008 | AI suggerimenti per aspetti | Richiede API funzionante |
| KEY-005 | Cmd+G genera AI | Richiede API funzionante |
| CTX-010 | Menu selezione: genera con AI | Richiede API funzionante |

### Mappa Radiale (Canvas)
| ID | Test | Motivo |
|----|------|--------|
| RAD-001 | Toggle vista radiale | Verifica visiva canvas |
| RAD-002 | Nodi mostrano struttura documento | Verifica visiva posizionamento |
| RAD-003 | Click su nodo seleziona chunk | Coordinate canvas dinamiche |
| RAD-004 | Zoom in/out funziona | Interazione gesture |
| RAD-005 | Pan (trascinamento) funziona | Interazione gesture |
| RAD-006 | Colori nodi per profondità | Verifica visiva colori |
| RAD-007 | Distribuzione leaf-weighted | Verifica visiva algoritmo |
| RAD-008 | Resize finestra aggiorna mappa | Verifica visiva responsiva |
| RAD-009 | Evidenziazione chunk corrente | Verifica visiva highlight |

### Performance (soggettiva)
| ID | Test | Motivo |
|----|------|--------|
| PERF-001 | Documento grande (100+ chunk) risponde | Percezione soggettiva |
| PERF-002 | Digitazione fluida senza lag | Percezione latenza |
| PERF-003 | Scroll outline fluido | Percezione fluidità |
| PERF-004 | Autosave non blocca UI | Percezione UI freeze |
| PERF-005 | Polling non blocca UI | Percezione UI freeze |
| PERF-006 | Mappa radiale fluida | Percezione animazione |
| PERF-007 | Import file grande (<5s per 1MB) | Misurazione tempo |
| PERF-008 | Export file grande (<5s per 1MB) | Misurazione tempo |

### Tauri (file system nativo)
| ID | Test | Motivo |
|----|------|--------|
| PROJ-002 | Apri progetto esistente (.trmd) | Dialog nativo file picker |
| PROJ-003 | Salva progetto (Cmd+S) | File system nativo |
| PROJ-004 | Salva con nome (Cmd+Shift+S) | Dialog nativo file picker |
| PROJ-005 | Dirty indicator (*) quando modificato | Stato file nativo |
| PROJ-006 | Conferma uscita con modifiche non salvate | Dialog nativo |
| ERR-003 | File non trovato gestito | File system nativo |
| ERR-004 | Permessi file insufficienti gestito | File system nativo |

### Altro
| ID | Test | Motivo |
|----|------|--------|
| BASIC-006 | Tutorial si avvia e naviga | Verifica visiva step-by-step |
| BASIC-007 | Keyboard shortcuts visibili in help | Verifica visiva lista |
| AUTH-011 | Rate limiting registrazione (max 5/ora per IP) | Richiede test prolungato |
| ERR-006 | Network offline gestito | Simulazione offline complessa |

**Totale test esclusivamente manuali:** ~35
